...
installPackages(){
    ...
    sudo apt install lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings lxsession-logout lxpolkit --no-install-recommends --no-install-suggests -y | tee -a obqss-setup.log
    sudo apt install libnotify-bin xdg-desktop-portal-gtk caffeine ffmpegthumbnailer xfce4-power-manager git tumbler synaptic obconf lxrandr eject bash-completion gvfs* qt5ct qt6ct adwaita-qt adwaita-qt6 qt5-gtk-platformtheme qt6-gtk-platformtheme openbox picom package-update-indicator network-manager network-manager-gnome xinit falkon pcmanfm l3afpad lxterminal flameshot lxappearance pulseaudio pamixer dunst pavucontrol engrampa mirage gmrun xserver-xorg xdg-user-dirs wpasupplicant btop xfce4-panel xfce4-whiskermenu-plugin xfce4-weather-plugin xfce4-pulseaudio-plugin xfce4-docklike-plugin arc-theme desktop-base xscreensaver papirus-icon-theme galculator flatpak fonts-noto* --no-install-recommends --no-install-suggests -y | tee -a obqss-setup.log
